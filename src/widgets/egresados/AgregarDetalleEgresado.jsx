...
import { FaUserPlus, FaTimes, FaBuilding, FaBriefcase, FaMapMarkerAlt, FaCalendarAlt, FaSpinner } from 'react-icons/fa';
...
function AgregarDetalleEgresado() {
  ...
  const { createDetalleEgresado } = useDetallesEgresados();
  ...
  // Pre-seleccionar el egresado si viene en los parámetros de la URL
  useEffect(() => {
    const codigoFromUrl = searchParams.get('codigo_egresado');
    if (codigoFromUrl && egresados.length > 0) {
      const egresadoSeleccionado = egresados.find(e => e.codigo === codigoFromUrl);
      if (egresadoSeleccionado) {
        setFormData(prev => {
          // Solo actualizar si no hay un código seleccionado
          if (!prev.codigo_egresado) {
            return {
              ...prev,
              codigo_egresado: codigoFromUrl
              // fecha_egreso se deja vacío para trabajos actuales
            };
          }
          return prev;
        });
  ...
  }, [searchParams, egresados.length]); // Solo depende del length para evitar loops
  ...
  const validateForm = () => {
    ...
    // Solo codigo_egresado es requerido según el backend
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await createDetalleEgresado(detalleData);

      if (result.success) {
        setMessage('Detalle de egresado creado exitosamente');
        // Limpiar formulario
        setFormData({
          codigo_egresado: '',
          fecha_egreso: '',
        ...
        setFormError(result.error || 'Error al crear el detalle de egresado');
      }
    } catch (err) {
  ...
  return (
    <div className="unified-content">
      <div className="unified-list">
        <TopHeader 
          title="Agregar Nuevo Detalle de Egresado"
          icon={<FaUserPlus />}
          subtitle="Complete la información del detalle laboral del egresado"
        />
